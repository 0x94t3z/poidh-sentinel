...
4. **Evaluates proof with AI** — deterministic pre-scorer + OCR + vision AI (Groq Llama 4 Scout) + LLM final verdict
...
1. **OCR** via [ocr.space](https://ocr.space) (free; `OCR_SPACE_API_KEY` optional, defaults to public key) — always runs, extracts text from images
2. **Vision AI** (only if deterministic score >= 40, `VISION_SCORE_GATE`):
   - Groq: `llama-4-scout-17b-16e-instruct` → `llama-3.2-90b-vision-preview` → `llama-3.2-11b-vision-preview`
   - OpenRouter fallback: `qwen/qwen3.6-plus:free` → `google/gemini-3.1-flash-lite-preview` → `openai/gpt-4o-mini`
...
Vision is skipped for low-scoring claims to conserve Groq quota. OCR is always run as it's free and quota-free.

If both vision and OCR are unavailable, the image URL itself is included for LLM context.
...
All LLM usage targets free-tier endpoints. No paid model is required.
...
| Tier | Provider   | Model(s)                                                                   |
|------|------------|----------------------------------------------------------------------------|
| 1    | Groq       | `llama-4-scout-17b-16e-instruct` → `llama-3.2-90b` → `llama-3.2-11b`     |
| 2    | OpenRouter | `qwen/qwen3.6-plus:free` → `google/gemini-3.1-flash-lite-preview`         |

**Groq quota note**: Groq vision and text calls share the same 500k TPD limit. To reduce vision usage, vision is skipped for claims with deterministic score < 40. If quota is exhausted mid-session, the evaluator falls back to OCR-only or deterministic score.
...
|--------------------------|-----------|---------------------------------------------------------------------------|
| `DATABASE_URL`           | Yes       | PostgreSQL connection string (auto-provisioned by Neynar Studio)          |
...
- Groq account — free-tier LLM + vision inference
- EVM wallet with ETH on Arbitrum or Base for gas (0.005 ETH recommended minimum)
- PostgreSQL database (auto-provisioned by Neynar Studio, or bring your own)
